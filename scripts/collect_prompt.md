...
- claude-in-chrome 스킬을 로드하고 새 탭에서
  ...
  예전 주소(`?q=공공AX&serp_type=tags`)는 2026-09-11 태그 탭이 사라지면서 1건만 돌려준다(아래 참조).
...
- 페이지를 2~3회 스크롤하며 게시물별로 (원문 텍스트, 게시물 링크, 작성일)을 읽는다.
  ...
  **피드 간헐 축소(2026-09-01~04, 시간대 불문 확인)**: 검색 피드가 게시물 1건만 렌더되는 현상이
  간헐적으로 나타난다(처음엔 심야에만 보였으나 09-04 아침·낮 회차에서도 재현). 재로드 1회가 무효면
  '부분 관측'으로 기록하고 넘어간다 — 정상 회차(20건)가 며칠 내 공백을 메우며, seen_ids 덕에 중복
  위험도 없다. 이틀 이상 전 회차가 연속 축소되면 검색 UI 변경을 스크린샷으로 진단한다.
  **진단 결과(2026-09-10, 3회차 연속 축소 후 실시)**: 검색 UI 변경은 아니다 — 태그 탭 구조(`공공AX`)는
  그대로이고, 스크롤을 추가로 내려도 `[data-pressable-container]`가 2개에서 늘지 않으며 body 텍스트가
  900자 남짓에 그친다. 즉 DOM 가상화나 추출 실패가 아니라 **태그 피드가 최근 게시물 2건만 반환**하는
  서버측 상태다. 대응은 그대로 '부분 관측' 기록 후 진행 — 카카오 공유 경로로 같은 게시물이 들어오는
  경우가 많고 seen_ids가 중복을 막는다. 축소가 더 길어지면 태그 대신 계정 타임라인 순회를 검토한다.
  **회복(2026-09-10 23:30 회차)**: 같은 날 밤 회차에서 20건이 정상 렌더돼 축소가 끝났다(연속 3회차로 종료).
  **채널 변경 확정(2026-09-11 23:51 회차)**: 축소가 아니라 검색 화면이 바뀌었다. 탭이
  `인기 검색 / 최근 검색 / 프로필`뿐이고 **`공공AX` 태그 탭 자체가 사라졌다**(스크린샷 확인).
  기존 주소 `?q=공공AX&serp_type=tags`는 이제 인기 검색으로 떨어져 1건만 돌려준다.
  - **쓸 주소**: `https://www.threads.com/search?q=%23공공AX` (해시태그 검색) — 20건이 나오고
    그중 본문에 공공AX가 있는 것이 선별 대상이다. 순수 태그 피드가 아니라 연관 검색이므로
    **게시물마다 공공AX 포함 여부를 보고 걸러야 한다**(민간 AX 경험담·코인 글이 섞여 든다).
  - **쓰면 안 되는 주소**: `https://www.threads.com/tag/공공AX` — 20건이 나와 정상처럼 보이지만
    **공공AX가 한 번도 안 나오는 무관한 추천 피드**다(2026-09-11 실측: 지캐시·트레이딩·OCR 글).
    건수만 보고 수집하면 아카이브가 오염된다.
...
- **구조 전제(2026-09-09 실측 확정)**: kakaocli는 앱이 관리하는 암호화 SQLite를 **읽기만** 한다(서버 통신 경로 없음 —
  업스트림 README도 "read commands work without the app open, but send·sync·harvest need the KakaoTalk window"로 명시).
  앱을 닫고 3분간 관측했을 때 DB 최신 메시지와 **파일 mtime이 완전히 정지**했고, 읽기 명령만은 정상 동작했다.
  즉 **읽기는 앱 없이 되지만 새 메시지를 DB에 채우는 주체는 앱 자신**이다 — 앱은 사실상 동기화 데몬이므로 항상 떠 있어야 한다.
  워치독이 이 전제를 떠맡는다(아래).
- **동기화 중단 판별**: kakaocli는 Mac 카카오톡 앱의 로컬 DB를 읽으므로 앱이 꺼져 있으면 새 메시지가 없다.
  ...
  **상시 자동 수집(2026-09-05~)**: 워치독이 정상 판정 시마다 최근 2일 창을 `data/raw/날짜-kakao-auto.json`으로
  원자적 덤프한다(30분 주기, 세션 크론과 무관). 커뮤니티 지표가 이 파일을 자동 집계하므로 대화량 원장은
  회차 공백·세션 종료와 무관하게 이어진다 — 정기 수집의 선별 절차(last_read 기준)는 그대로 유지한다.
  워치독(`scripts/kakao_watchdog.sh`, launchd **10분** 주기)이 3층으로 자가복구한다 —
  ① 프로세스 감시: 앱이 종료돼 있으면 즉시 백그라운드로 재기동(`open -g` — 화면 전환 없음)하고
  DB 신선도로 동기화가 붙었는지 확인한다(안 붙으면 창 활성화로 기동을 마무리). 사용자가 ⌘Q로 닫아도
  10분 안에 복구된다. **창을 감춘 채(`open -j`) 띄우면 앱이 launching에 머물러 동기화가 붙지 않으므로
  숨김 기동은 쓰지 않는다**(2026-09-09 실측: 72분 정체 → 창 활성화 직후 즉시 따라잡음).
  `kakaocli status`의 App state는 동기화 정상에도 launching을 보고해 판정 근거로 쓰지 않는다. ② 로그인 감시: 명시적 로그아웃 상태면 재기동으로 못 고치니
  사용자 알림으로 넘긴다(launching 같은 과도기 상태는 오탐 방지를 위해 한 번 더 확인). ③ 정체 감시: 전 방 45분+
  정체 시 재실행 → 지속되면 알림(재실행 가드 45분). 동시 실행 락으로 launchd 주기 실행과 수동 실행이 서로의 판정을 오염시키지 않는다.
  **재실행은 종료·기동을 프로세스로 확인한다(2026-09-10 수정)**: `osascript quit`는 앱이 굳으면 애플 이벤트
  타임아웃까지 매달리고, 종료가 끝나기 전에 `open`을 던지면 LaunchServices가 죽어 가는 인스턴스를 붙잡아
  기동이 통째로 흘러간다. `open`의 종료코드는 '요청 접수'일 뿐 기동 성공이 아니므로 `pgrep`으로 확인해야 한다.
  이 확인이 없던 탓에 11시간(03:27Z~14:35Z) 정체가 났고 로그에는 '앱 기동 완료'가 계속 찍혔다 —
  실제로는 앱이 죽어 있었다. 앱이 완전히 내려간 상태에서 새로 띄우면 2~3분 안에 밀린 메시지까지 따라잡는다.
  중단이 관측되면 `data/private/watchdog.log`를 함께 확인해 조치 이력을 로그에 남긴다.
